...
清洗 data_raw/etmall/Order_Report 下所有檔案及下面資料夾內檔案
只保留指定的 14 個欄位：
...
def is_valid_order_sn(order_sn: str) -> bool:
    ...
    # 檢查是否只包含英文文字和數字
    if not re.match(r'^[a-zA-Z0-9]+$', order_sn):
        return False
...
def clean_order_report_file(file_path: Path, temp_dir: Path) -> bool:
    ...
    try:
        ...
        if missing_columns:
            logging.warning(f"缺少以下欄位：{missing_columns}")
...
def find_order_report_files(order_report_dir: Path) -> List[Path]:
    """尋找 Order_Report 目錄下的所有 CSV 檔案"""
    csv_files = []
    
    # 遞迴搜尋所有 CSV 檔案
    for file_path in order_report_dir.rglob("*.csv"):
        # 排除備份檔案
...
def main() -> None:
    ...
    order_report_dir = project_root / 'data_raw' / 'etmall' / 'Order_Report'
    temp_dir = project_root / 'temp' / 'etmall'
    
    logging.info(f'專案根目錄：{project_root}')
    logging.info(f'訂單報表目錄：{order_report_dir}')
    logging.info(f'輸出目錄：{temp_dir}')
    
    # 檢查目錄是否存在
    if not order_report_dir.exists():
        logging.error(f"訂單報表目錄不存在：{order_report_dir}")
        return
    ...
    # 尋找所有 CSV 檔案
    csv_files = find_order_report_files(order_report_dir)
