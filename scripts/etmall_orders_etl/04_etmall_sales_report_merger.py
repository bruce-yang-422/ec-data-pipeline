...
def process_csv_file(file_path, logger):
    ...
    try:
        ...
        df['訂單ID'] = df['訂單編號'].astype(str) + '_' + df['項次'].astype(str).str.zfill(2)
        
        # 重新排列欄位順序：在 "項次" 和 "配送狀態" 之間插入 "訂單ID"
        columns = list(df.columns)
        order_index = columns.index('項次')
...
def main():
    ...
    # 設定路徑
    base_dir = "data_raw/etmall/sales_report"
