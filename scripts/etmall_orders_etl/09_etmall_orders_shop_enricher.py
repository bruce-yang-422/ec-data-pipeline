...
"""
東森購物訂單商店資料豐富腳本
根據 shop_id 從 config/A02_Shops_Master.json 填入相應的商店資料
"""
...
def setup_logging():
    ...
    log_file = log_dir / f"etmall_orders_shop_enricher_{timestamp}.log"
...
def load_shops_master(config_path, logger):
    """載入商店主檔配置"""
    try:
        logger.info(f"正在載入商店主檔配置: {config_path}")
        
        with open(config_path, 'r', encoding='utf-8') as f:
            shops_master = json.load(f)
        
        # 創建 shop_id 到商店資料的映射字典
        shops_dict = {}
        for shop in shops_master.get('shops', []):
            shop_id = shop.get('shop_id')
            if shop_id:
                shops_dict[shop_id] = shop
        
        logger.info(f"成功載入商店主檔配置，共 {len(shops_dict)} 個商店定義")
        return shops_dict
        
    except Exception as e:
        logger.error(f"載入商店主檔配置失敗: {str(e)}")
        return None
...
def enrich_shop_data(df, shops_dict, logger):
    """根據 shop_id 豐富商店資料"""
    try:
        logger.info("開始豐富商店資料...")
        
        # 複製資料框以避免修改原始資料
        enriched_df = df.copy()
        
        # 檢查是否有 shop_id 欄位
        if 'shop_id' not in enriched_df.columns:
            logger.error("資料中沒有 'shop_id' 欄位")
            return None
        
        # 獲取唯一的 shop_id 值
        unique_shop_ids = enriched_df['shop_id'].unique()
        logger.info(f"發現的 shop_id 值: {list(unique_shop_ids)}")
        
        # 定義要填入的欄位映射
        field_mapping = {
            'shop_name': 'shop_name',
            'shop_business_model': 'shop_business_model',
            'location': 'location',
            'phone': 'phone',
        ...
        # 為每個 shop_id 填入相應的商店資料
        for shop_id in unique_shop_ids:
            if shop_id in shops_dict:
                shop_data = shops_dict[shop_id]
                logger.info(f"正在處理 shop_id: {shop_id}")
                
                # 填入商店資料
                for target_field, source_field in field_mapping.items():
                    if source_field in shop_data:
                        # 使用 loc 來避免 SettingWithCopyWarning
                        mask = enriched_df['shop_id'] == shop_id
                        enriched_df.loc[mask, target_field] = shop_data[source_field]
                        logger.info(f"  已填入 {target_field}: {shop_data[source_field]}")
                    else:
                        logger.warning(f"  商店資料中沒有 {source_field} 欄位")
            else:
                logger.warning(f"未找到 shop_id {shop_id} 的商店資料")
        
        logger.info("商店資料豐富完成")
        return enriched_df
        
    except Exception as e:
        logger.error(f"豐富商店資料失敗: {str(e)}")
        return None
...
def save_enriched_file(df, output_dir, logger):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = output_dir / f"etmall_orders_shop_enriched_{timestamp}.csv"
...
def analyze_enrichment_results(df, logger):
    ...
    try:
        ...
        # 檢查新增的商店相關欄位
        shop_related_fields = [
            'shop_name', 'shop_business_model', 'location', 'phone', 
            'department', 'manager'
        ]
        
        logger.info("商店相關欄位統計:")
        for field in shop_related_fields:
...
def cleanup_temp_files(temp_dir, logger, latest_output_files):
    ...
    try:
        ...
        patterns_to_delete = [
            ...
            "etmall_orders_shop_enriched_*.csv"             # 09 腳本過去輸出
        ]
...
def main():
    ...
    logger.info("開始執行東森購物訂單商店資料豐富腳本")
    
    # 設定路徑
    shops_master_path = "config/A02_Shops_Master.json"
    temp_dir = "temp/etmall"
    output_dir = "temp/etmall"
    
    # 檢查配置檔案是否存在
    if not os.path.exists(shops_master_path):
        logger.error(f"商店主檔配置不存在: {shops_master_path}")
        return
    
    # 載入商店主檔配置
    logger.info("=" * 50)
    logger.info("載入商店主檔配置")
    logger.info("=" * 50)
    
    shops_dict = load_shops_master(shops_master_path, logger)
    if shops_dict is None:
        logger.error("商店主檔配置載入失敗")
        return
    ...
    # 找尋 08 腳本的最新檔案
    mapped_pattern = os.path.join(temp_dir, "etmall_orders_field_mapped_*.csv")
    latest_mapped_file = find_latest_file(mapped_pattern, logger)
    
    if not latest_mapped_file:
    ...
    df = load_file(latest_mapped_file, logger)
    if df is None:
        logger.error("檔案載入失敗")
        return
    ...
    enriched_df = enrich_shop_data(df, shops_dict, logger)
    if enriched_df is None:
        logger.error("豐富商店資料失敗")
        return
